...
class MilvusStorage(BaseService):
        ...
        if self.client.has_collection(self.collection_name):
            ...
            for field in info.get("fields", []):
                if field.get("type") == DataType.FLOAT_VECTOR:
                    vector_dim = int(field.get("params", {}).get("dim", 0))
                    break
            expected = int(self.settings.embedding_dimensions)
            if vector_dim and vector_dim != expected:
                self.logger.warning(
                    "Recreating local Milvus collection due to dimension mismatch",
    ...
    def _create_collection_local(self):
        ...
        self.logger.info("Created local collection", name=self.collection_name, dimension=self.settings.embedding_dimensions)
    ...
    def _create_collection_server(self):
        ...
        fields = [
            ...
            FieldSchema(name="embedding", dtype=DataType.FLOAT_VECTOR, dim=self.settings.embedding_dimensions),
    ...
    async def insert_embeddings(
        ...
        try:
            if self.mode == MilvusMode.LOCAL:
                ...
                # 生产模式 - 使用DataFrame按列插入，确保类型匹配
                try:
                    import pandas as pd
                except ImportError as exc:  # pragma: no cover - sanity guard
                    raise StorageError(f"Pandas not installed: {exc}", "insert") from exc
                ...
                frame = pd.DataFrame(
                    {
                        "id": [str(chunk["id"]) for chunk in chunks],
                        "document_id": [str(chunk["document_id"]) for chunk in chunks],
                        "content": [chunk["content"][:50000] for chunk in chunks],
                        "embedding": embeddings,
                        "chunk_type": [chunk["chunk_type"] for chunk in chunks],
                        "position": [int(chunk["position"]) for chunk in chunks],
                    }
                )[[
                    "id",
                    "document_id",
                ...
                result = self.collection.insert(frame)
