...
class BaseParser(ABC):
    ...
    async def parse_async(self, file_path: str) -> str | None:
        """
        异步解析文档并提取纯文本内容。

        默认实现：使用 asyncio.to_thread 包装同步 parse 方法。
        子类可以重写此方法以实现真正的异步解析。
        ...
        Returns:
            提取的纯文本内容，解析失败时返回None
        """
        # 默认实现：在线程池中运行同步方法，避免阻塞事件循环
        return await asyncio.to_thread(self.parse, file_path)
