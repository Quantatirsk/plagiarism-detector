...
class ParagraphOptimizer:
    ...
    def fix_text_line_breaks(self, text: str) -> str:
        ...
        for line in lines:
            ...
            if self.should_start_new_paragraph(line, current_paragraph) and current_paragraph:
                paragraph_text = self.join_paragraph_lines(current_paragraph)
                if paragraph_text:
                    result.append(paragraph_text)
                current_paragraph = []
    ...
    def join_paragraph_lines(self, lines: list[str]) -> str:
        """
        使用语言感知的智能行合并。
    ...
    def should_start_new_paragraph(self, line: str, current_paragraph: list[Any]) -> bool:
        ...
        return any(re.match(pattern, line) for pattern in paragraph_starters)
    ...
    def should_join_lines(self, prev_line: str, current_line: str) -> bool:
        ...
        return bool(re.search(r'[，,、]$', prev_line))
    ...
    def optimize_text(self, text: str, options: dict[str, Any] | None = None) -> str:
        """
        主要的文本优化入口方法。
        ...
        options.get('preserve_tables', True)
